    ...
    <>
      <section className="brand-story" id="about">
        <div className="story-heading">
          <p className="eyebrow">{config.showcase.badge}</p>
          <h2>{config.title ?? config.showcase.title}</h2>
          <p>{config.subtitle ?? config.showcase.description}</p>
        </div>
        <div className="story-copy">
          <p>{config.content ?? config.showcase.description}</p>
          <div className="owner-signature">
            <strong>{config.owner.name}</strong>
            <span>{config.owner.handle}</span>
          </div>
        </div>
      </section>
      <footer className="store-footer" id="contact">
        <div className="footer-brand">
          <strong>{config.title ?? config.showcase.title}</strong>
        ...
        <div className="footer-column">
          {config.contactPersonName && (
            <strong>{config.contactPersonName}</strong>
          )}
          {config.contactPersonEmail && (
            <a href={`mailto:${config.contactPersonEmail}`}>
              {config.contactPersonEmail}
            </a>
          )}
          {config.contactPersonPhone && (
            <a href={`tel:${config.contactPersonPhone}`}>
              {config.contactPersonPhone}
            </a>
          )}
          {config.contactPersonWhatsapp && (
            <a
              href={`https://wa.me/${config.contactPersonWhatsapp}`}
              target="_blank"
              rel="noreferrer"
            >
              {config.contactPersonWhatsapp}
